...
public class VW_CRC16
        {
            public static int[] cp_Crc16_CCITT =
            {
            ...
            public static int CRC16CCITT(byte[] bytes, int initialValue)
            {
                final int poly = 4129;
                ...
                for (int i = 0; i < bytes.length; ++i)
                {
                    crc = (int)((crc << 8) ^ table[((crc >> 8) ^ (0xff & bytes[i]))]);
                }
                return crc;
            }
